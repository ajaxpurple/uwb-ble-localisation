...
void Blink_Task(void){
  ...
  BLEPacket packet;
  uint8_t disabled = 0;
  s4436572_hal_mouse_init();
  xSemaphoreGive(s4436572_SemaphoreBLEAdvertiseStart);
  for(;;){
    if(xSemaphoreTake(s4436572_SemaphoreBLEAccelToggle, 10)){
      disabled ^= 1;
    }
    ...
    if(disabled == 0){
      ...
      packet.major = xyzValues.x;
	  packet.minor = xyzValues.y;
	  s4436572_hal_mouse_move(((int16_t)xyzValues.x) >> 5, ((int16_t)xyzValues.y) >> 5);
