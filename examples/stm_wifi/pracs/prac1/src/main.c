...
int main( void ) {
    ...
    s4436572_os_time_init(); //Initialise radio
...
void Blink_Task(void){
	...
	int count = 0;
  for(;;){
	if(xSemaphoreTake(s4436572_SemaphoreLedToggleBlink, 0) == pdTRUE){
		disabled ^= 1;
	}
	if(disabled == 0){
		count++;
		s4436572_hal_led_toggle(0);
		console_log(ERR, "LED flash %d\r\n", count/2);
		console_log(LOG, "LED toggle %d\r\n", count);
		console_log(DEBUG, "Time: %dms, Memory: %d\r\n", xTaskGetTickCountFromISR(), xPortGetFreeHeapSize());
	}
	vTaskDelay(100);
  }
}
